...
// Holiday markers (month 0-indexed → day → short label)
const HOLIDAYS = {
  0:  { 1:  "New Year's Day" },
...
function CalendarGrid({ monthIndex, rangeStart, rangeEnd, selecting, onDayClick, onDayHover }) {
  const grid     = useMemo(() => buildGrid(monthIndex), [monthIndex]);
  const holidays = HOLIDAYS[monthIndex] || {};

  const today    = new Date();
  const todayDay = today.getFullYear() === YEAR && today.getMonth() === monthIndex
    ? today.getDate()
    : null;
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      >
        {grid.map((day, i) => {
          ...
          const inRng   = !isStart && !isEnd && isInRange(key, rangeStart, rangeEnd);
